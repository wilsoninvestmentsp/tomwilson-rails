class SyndicationsController < ApplicationController
  ...
  def index
    @syndications = Syndication.where(status: ['active', 'currently_held', 'sold']).order(:status).by_user(current_user).page(params[:page]).per(Settings.pagination.syndication.per_page)
  end
